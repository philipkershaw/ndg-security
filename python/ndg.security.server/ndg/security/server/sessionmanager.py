...
class UserSession(object):
    ...
    def __init__(self, **credentialWalletKeys):
        """Initialise UserSession with keywords to CredentialWallet"""
        ...
        self.__sessIdList = []
        self.addNewSessID()
        self.__credentialWallet = CredentialWallet(**credentialWalletKeys)
    ...
    credentialWallet = property(fget=__getCredentialWallet,
                          doc="Read-only access to CredentialWallet instance")
    ...
    @staticmethod
    def encodeTxt(txt, encrKey=None):
        ...
        if encrKey is not None:
            ...
            try:
                ...
                raise UserSessionError("Encrypting text: %s" % e)
    ...
    @staticmethod                                   
    def decodeTxt(txt, encrKey=None):
        ...
        try:
            ...
            raise SessionManagerError("Decoding text: %s" % e)          
        if encrKey is not None:
            try:
                ...
                raise SessionManagerError("Decrypting Session Manager URI: %s" % e)           
        else:
...
class SessionManager(dict):
    ...
    def __init__(self, 
        ...
        # otherwise explicit calls are necessary to set credentialRepositoryProp via
        # setProperties/readProperties and then loadCredentialRepositoryInterface
    ...
    def setPropPrefix(self, val=None):
        ...
        log.debug("Setting property file section name")
        if val is None:
            val = os.environ.get('NDGSEC_SM_PROPFILEPREFIX', 'DEFAULT')
    ...
    def readProperties(self, section=None, prefix=None):
        ...
        # Keep a copy of the config file for the CredentialWallet to reference 
...
#                        
    ...
    def connect(self, 
        ...
        if sessID is not None:            
            ...
            if createServerSess:
                ...
                userSess = self._createUserSession(username, 
                                                   passphrase,
                                                   userCreds)
                sessID = userSess.latestSessID
            else:
    ...
    def _createUserSession(self, username, userPriKeyPwd=None, userCreds=None):
        ...
        log.debug("Calling SessionManager._createUserSession ...")
        ...
        if username in self.__usernameDict:
            ...
            try:   
                ...
                log.error("Error occurred whilst creating User Session: %s"%e)
                raise 
    ...
    def _connect2UserSession(self,username=None,userX509Cert=None,sessID=None):
        ...
        try:
            userSess.credentialWallet.isValid(raiseExcep=True)
            return userSess
    ...
    def getAttCert(self,
        ...
        @param **credentialWalletKw: keywords to CredentialWallet.getAttCert
        """
...
class AuthNServiceError(Exception):
    ...
    msg = "An error occured with login"
...
class AuthNServiceRetrieveError(AuthNServiceError):
    ...
    msg = \
    "An error occured retrieving information to check the login credentials"

class AuthNServiceInitError(AuthNServiceError):
    """Error with initialisation of AuthNService.  Raise from __init__"""
    msg = "An error occured with the initialisation of the Session " + \
...
class AuthNServiceConfigError(AuthNServiceError):
    ...
    msg = "An error occured with the Session Manager's Authentication " + \
