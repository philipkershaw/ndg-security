#
# ESG Recursive Download script wraps wget call with settings for ESG Security
#
...
# Two stage wget call to workaround recursive mode (-r) not working with 
# redirects:
#
# 1) Authenticate - this involves redirects to and back from SSL endpoint 
#    submitting a certificate and saving a session cookie.  
wget --spider \
 --ca-directory=$caDir \
...
# 2) Call again with session cookie to allow for the recursive case - with a 
#    cookie set, no redirects are invoked and so in this case recursive download 
#    will succeed.  If the request was for a single file, step 1 would have 
