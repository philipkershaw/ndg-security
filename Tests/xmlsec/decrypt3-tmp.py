#!/usr/bin/env python
#
# $Id: decrypt3.py,v 1.4 2004/01/25 00:31:12 valos Exp $
#
...
def main():
    ...
    # Add private key file as command line argument
    #
    # PJK
    if len(sys.argv) != 3:
#    if len(sys.argv) != 2:
...
#        print "Usage: %s <enc-file>" % sys.argv[0]
        print "Usage: %s <enc-file> <private key file>" % sys.argv[0]
...
# Load private key into a simple keys manager
# 
# PJK
def loadPriKey(file):
    assert(file)
    ...
    key = xmlsec.cryptoAppKeyLoad(file, xmlsec.KeyDataFormatPem, None, None, None)
    if key is None:
        print "Error: failed to load private key from file \"%s\"" % file
        mngr.destroy()
        return None
