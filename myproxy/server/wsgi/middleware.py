    ...
    def myProxyLogon(self):
        ...
        def _myProxylogon(environ, start_response, username, password):
            ...
            if requestMethod == 'POST':
                ...
                # Alternative, GET method would set this variable, set to None
                # here to correctly initialise MyProxy.logon call
                pubKey = None
                
            elif requestMethod == 'GET':
                # Interpret as a HTML keygen request.  See:
                # https://developer.mozilla.org/en/HTML/Element/keygen
                # A Netscape Signed Public Key and Challenge (SPKAC) is set as a
                # query argument
                spkac = request.GET.get(certReqKey)
                if spkac is None:
                    response = ("No %r query argument set in GET request" % 
                                certReqKey)
                    log.error(response)
                    raise HttpBasicAuthResponseException(response, 
                                                         httplib.BAD_REQUEST) 
                
                # Remove any carriage returns to enable PyOpenSSL interface to
                # correctly parse
                strippedSpkac = ''.join(re.split('\s+', spkac))
                try:
                    spki = crypto.NetscapeSPKI(strippedSpkac)
                    
                except crypto.Error, e:
                    log.error("Error loading input signed public key and "
                              "challenge: %r", spkac)
                    raise HttpBasicAuthResponseException("Error loading input "
                                                         "Signed Public Key "
                                                         "and challenge",
                ...
                # Extract public key 
                try:
                    pubKey = spki.get_pubkey()
                    
                except crypto.Error, e:
                    log.error("Error extracting public key from input "
                              "signed public key and challenge: %r", spkac)
                    raise HttpBasicAuthResponseException("Error procesing "
                                                         "input Signed Public "
                                                         "Key and challenge",
                                                         httplib.BAD_REQUEST)
                    
                # Alternative, POST method would set this variable, set to None
                # here to correctly initialise MyProxy.logon call
