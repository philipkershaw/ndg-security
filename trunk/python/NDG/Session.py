...
#_____________________________________________________________________________
class _MetaUserSession(type):
    """Enable UserSession to have read only class variables e.g.
    
    print UserSession.cookieTags is allowed but,
    
    UserSession.cookieTags = None
    
    ... raises - AttributeError: can't set attribute"""
    def __getCookieTags(cls):
        return ("NDG-ID1", "NDG-ID2")

    cookieTags = property(fget=__getCookieTags)
...
class UserSession(object):
    ...
    __metaclass__ = _MetaUserSession
    ...
    cookieDomain = property(fset=__setCookieDomain, doc="Set cookie domain")
    ...
    def addNewSessID(self):
        ...
        sessID = base64.urlsafe_b64encode(os.urandom(self.__sessIDlen))
    ...
    def encrypt(txt, encrKey):
        ...
        try:
            ...
            return base64.urlsafe_b64encode(aes.encrypt(paddedURI))
    ...
    def decrypt(encrTxt, encrKey):
        ...
        try:
            ...
            # Decode from base 64
            b64DecodedEncrTxt = base64.urlsafe_b64decode(encrTxt)
    ...
    def createCookie(self, 
        ...
        encrSessMgrWSDLuri = self.encrypt(sessMgrWSDLuri, encrKey)
        dtExpiry = self.credWallet.proxyCert.notAfter
        
        # Call class method 
        return self.__class__.createSecurityCookie(sessID, 
                                            encrSessMgrWSDLuri,
                                            dtExpiry=dtExpiry,
                                            cookieDomain=self.__cookieDomain,
                                            asString=asString)
        
    
    #_________________________________________________________________________    
    @classmethod
    def createSecurityCookie(cls, 
                             sessID, 
                             encrSessMgrWSDLuri,
                             expiryStr=None,
                             dtExpiry=None,
                             cookieDomain=None,
                             asString=False):
        """Class method for generic cookie creation independent of individual
        UserSession details.  This is useful for making a fresh cookie from
        an existing one.
        
        sessID:                session ID for cookie
        encrSessMgrWSDLuri:    encrypted Session Manager WSDL address.
                               Use UserSession.encrypt().  The site 
                               SessionManager holds the encryption key.
        expiryStr|dtExpiry:    expiry date time for the cookie.  Input as a 
                               string formatted in the standard way for 
                               cookies or input a datetime type for 
                               conversion.
        cookieDomain:          The domain for the cookie.  Default is the
                               web server address.  Override to set to a 
                               site wide cookie e.g. .rl.ac.uk
        asString:              a SimpleCookie instance is returned be default.
                               Set this flag to True to override and return
                               the string text instead.
        """
        
        if len(sessID) < cls.__sessIDlen:
            UserSessionError, "Session ID has an invalid length"
            
        if encrSessMgrWSDLuri[:7] == 'http://' or \
           encrSessMgrWSDLuri[:8] == 'https://':
            UserSessionError, "Input Session Manager WSDL URI does not " + \
        ...
        elif not expiryStr or not isinstance(expiryStr, basestring):
            UserSessionError, "No cookie expiry was set"
        ...
        try:   
            ...
            i=0
            for tag in cls.cookieTags:
                
                sessCookie[tag] = tagValues[i]
                i += 1
                
                # Use standard format for cookie path and expiry
                sessCookie[tag][cls.__cookiePathTag] = cls.__cookiePath                
                sessCookie[tag][cls.__cookieExpiryTag]= expiryStr
                                            
                # Make cookie as generic as possible for domains - Nb. '.uk'
                # alone won't work but .rl.ac.uk would
                if cookieDomain:
                    sessCookie[tag][cls.__cookieDomainTag] = cookieDomain
    ...
    #_________________________________________________________________________    
    @classmethod
    def isValidSecurityCookie(cls, cookie, raiseExcep=False):
        ...
        if isinstance(cookie, basestring):
            ...
            if raiseExcep:
                raise UserSessionError, "Input cookie must be a string or " +\
                                        "SimpleCookie type"
            else:
                return False
        
        missingTags = [tag for tag in cls.cookieTags if tag not in cookie]
        if missingTags:
            if raiseExcep:
                raise UserSessionError, \
                    "Input cookie missing security tag(s): " + \
                    ", ".join(missingTags)
            else:
                return False

        if len(cookie[cls.cookieTags[0]].value) < cls.__sessIDlen:
            if raiseExcep:
                raise UserSessionError, "Session ID has an invalid length"
            else:
                return False
...
class SessionMgr(dict):
    ...
    def readProperties(self,
        ...
        try:
            for elem in propElem:
                if elem.tag == 'myProxyProp':
                    ...
                    # field as pass-phrase might contain leading or 
                    # trailing white space
                    if elem.text and elem.tag != 'keyPPhrase':
        ...
        if missingKeys != []:
            raise SessionMgrError, "The following properties are " + \
    ...
    def addUser(self, caConfigFilePath=None, caPassPhrase=None, **reqKeys):        
        ...
        **reqKeys:                      use as alternative to 
        ...
        try:
            ...
            #
            # Changed so that record is NOT added to UserID table of 
            # CredentialRepository.  Instead, a check can be made when a new
            # Attribute Certificate credential is added: if no entry is 
            # present for the user, add them into the UserID table at this 
            # point.
            #
            # By removing the add record call, MyProxy and Credential
            # Repository can be independent of one another
    ...
    def connect(self, **reqKeys):        
        ...
        if 'sessID' in reqKeys:
            ...
            try:            
                ...
                # session
                proxyCert = self.__myPx.getDelegation(reqKeys['userName'], 
                                                      reqKeys['pPhrase'])    
            except Exception, e:
    ...
    def __createUserSession(self, proxyCert):
        ...
        try:   
            ...
            try:
                userDN = str(X509CertParse(proxyCert).dn)
                
            except Exception, e:
                raise SessionMgrError, \
                "Parsing input proxy certificate DN for session create: %s"%\
                                                                        str(e)

            if userDN in self.__dnDict:
                # Update existing session with proxy cert and add a new 
            ...
            raise SessionMgrError, "Creating User Session: %s" % str(e)
    ...
    def __connect2UserSession(self, **idKeys):
        ...
        proxyCert:    proxy certificate string corresponding to an existing 
        ...
        if 'sessID' in idKeys:
            try:
                ...
                userSess = self.__sessDict[idKeys['sessID']]
            ...
            try:
                userDN = str(X509CertParse(idKeys['proxyCert']).dn)
            ...
            raise SessionMgrError,\
                                '"sessID" or "proxyCert" keywords must be set'


    #_________________________________________________________________________        
    def deleteUserSession(self, sessID=None, proxyCert=None):
        """Delete an existing session by providing a valid session ID or
        proxy certificate - use for user logout

        __deleteUserSession([proxyCert]|[sessID])
        ...
        if sessID:
            ...
            userDN = userSess.credWallet.proxyCert.dn
        ...
        try:
            ...
            for userSessID in userSess.sessIDlist:
                del self.__sessDict[userSessID]

            del self.__dnDict[userDN]
    ...
    def reqAuthorisation(self, **reqKeys):
        ...
        **reqKeys:            pass equivalent to XML as keywords instead.
        ...
        if 'sessID' in reqKeys:
            ...
            raise SessionMgrError,'Expecting "sessID" or "proxyCert" keywords'
        ...
        if 'aaPubKey' not in reqKeys:
            ...
            try:
                aaClnt = AttAuthorityClient(aaWSDL=reqKeys['aaWSDL'])                
                reqKeys['aaPubKey'] = aaClnt.getPubKey()
...
class SessionMgrCredRepos(CredRepos):
    ...
    def __setConnection(self,
        ...
        SessionMgrCredRepos.UserID._connection = self.__con
    ...
    def addCredentials(self, dn, attCertList):
        ...
        try:
            ...
            raise CredReposError("Checking for user \"%s\": %s"%(dn, str(e)))
