...
class XMLMsg(dict):
    ...
    def __init__(self, 
        ...
        **xmlTags:             keywords corresponding to the XML tags to be 
        ...
        if encrXMLtxt:            
            ...
            # XML text or XML tags input 
            if xmlTxt:
                # Input XML text set - parse tags into dictionary 
                # self.__xmlTags
                if encrPriKeyFilePath:
                    # decrypt if a private key was set
                    try:
                        self.decrypt(encrPriKeyPwd, xmlTxt)
                    except:
                        # Try parse without encryption
                        self.__xmlTxt = xmlTxt
                else:
                    self.__xmlTxt = xmlTxt
    ...
    # There doesn't seem to be an implicit function to allow a dict subclass
    # to yield a dictionary for dict(obj) - Make a 'xmlTags' property instead
    def __getXMLtags(self):
        """Return dictonary of XML tags"""
        return self.__xmlTags
    
    xmlTags = property(fget=__getXMLtags, doc="XML tags as a dictionary")
    ...
    def update(self, xmlTagsDict=None, **xmlTags):
        ...
        if unknownKeys:
            raise KeyError, "Invalid keywords set for update: \"%s\"" % \
                            '", "'.join(unknownKeys)
    ...
    def updateXML(self, **xmlTags):
        ...
        try:
            ...
            for tag in self.__xmlTags:
                ...
                if self.__xmlTags[tag] is None:
                    continue
                
                elif isinstance(self.__xmlTags[tag], bool):                    
    ...
    def chkTags(self):
        ...
        missingTags=[tag for tag in self.xmlMandatoryTags if not tag in self]
        if missingTags:
            raise XMLMsgError(\
    ...
    #_________________________________________________________________________
    def encrypt(self, encrPubKeyFilePath=None):
        """Encrypt message"""
    ...
    #_________________________________________________________________________
    def decrypt(self, encrPriKeyPwd, xmlTxt=None):
        """Decrypt encrypted text"""
        
        if xmlTxt is None:
            xmlTxt = self.__encrXMLtxt
