"""4Suite XML substitution for default DOM parsing and outputting for 
ZSI based services.

This code is included for NDG Security code from pyGridWare package by 
permission of the authors.  See copyright notice below.
"""
__author__ = "Joshua R. Boverhof, LBNL"
__date__ = "04/03/09"
__copyright__ = (
...
class DomletteReader(NonvalidatingReaderBase):
    ...
    def __init__(self, *arg, **kw):
        NonvalidatingReaderBase.__init__(self, *arg, **kw)
        self._context = None
        self.processorNss = None
...
#            return Canonicalize(self.node)
    ...
    def canonicalize(self, algorithm=DSIG.C14N, unsuppressedPrefixes=[]):
        '''4Suite-XML based canonicalization'''
        f = StringIO()
        if algorithm == DSIG.C14N_EXCL:
            CanonicalPrint(self.node, stream=f, exclusive=True,
                           inclusivePrefixes=unsuppressedPrefixes)
        else:
            CanonicalPrint(self.node, stream=f)
