...
xmlSecKeysMngrPtr load_rsa_keys(char* key_file, const char *szPriKeyFilePath);
...
main(int argc, char **argv) {
    ...
    mngr = load_rsa_keys(argv[2], "badc-aa-key.pem");
    if(mngr == NULL) {
	return(-1);
    }
    ...
    */
    if (encrypt_file(mngr, argv[1], "badc-aa-key.pem") < 0) 
    {
 ...
 */
xmlSecKeysMngrPtr 
load_rsa_keys(char* key_file, const char  *szPriKeyFilePath) 
{
    xmlSecKeysMngrPtr mngr;
    xmlSecKeyPtr key;
    ...
    */
    key = xmlSecCryptoAppKeyLoad(key_file, xmlSecKeyDataFormatCertPem, NULL, NULL, NULL);
    if(key == NULL) {
