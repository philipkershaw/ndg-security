...
class CredWallet(object):
    ...
    def __reqAuthorisation(self,
        ...
        if aaWSDL is not None:
            ...
            try:
                ...
                raise CredWalletError(\
                    "ServiceProxy for authorisation request: " + str(e))
                              
                              
            if aaPubKeyFilePath is None:
                # Try retrieving from the web service
                try:
                    pubKeyReq = PubKeyReq()
            ...
            if self.__clntPubKeyFilePath:
                try:
                    clntCert = open(self.__clntPubKeyFilePath).read()
            ...
            try:                                
                ...
                #
                # Message will be encrypted if aaPubKeyFilePath was set
                authorisationReq = AuthorisationReq(\
                                        proxyCert=self.__proxyCertTxt,
                                        userAttCert=extAttCert,
                                        encrCert=clntCert,
                                        encrPubKeyFilePath=aaPubKeyFilePath)
            ...
            authorisationResp = AuthorisationResp(\
                                 xmlTxt=resp['authorisationResp'],
                                 encrPriKeyFilePath=self.__clntPriKeyFilePath,
                                 encrPriKeyPwd=self.__clntPriKeyPwd)
    ...
    def getAATrustedHostInfo(self,
        ...
        if userRole is not None and not isinstance(userRole, basestring):
        ...
        if aaWSDL is not None:
            ...
            if self.__clntPubKeyFilePath:
                # Read client certificate into a string ready to pass over 
                # SOAP connection
                try:
                    clntCert = open(self.__clntPubKeyFilePath).read()
