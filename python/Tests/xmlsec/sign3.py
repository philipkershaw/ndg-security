#!/usr/bin/env python
#
# $Id: sign3.py,v 1.4 2004/01/25 00:31:12 valos Exp $
#
...
def sign_file(xml_file, key_file, cert_file):
    ...
    # P J Kershaw 11/08/05 -add extra X509Data tags
    x509DataNode = keyInfoNode.addX509Data()
    if x509DataNode is None:
        print "Error: failed to add X509Data node"
        return cleanup(doc)

    if xmlsec.addChild(x509DataNode, xmlsec.NodeX509Certificate) is None:
        print "Error: failed to add %s node" % xmlsec.NodeX509Certificate
        return cleanup(doc)

    if xmlsec.addChild(x509DataNode, xmlsec.NodeX509SubjectName) is None:
        print "Error: failed to add %s node" % xmlsec.NodeX509SubjectName
        return cleanup(doc)

    if xmlsec.addChild(x509DataNode, xmlsec.NodeX509IssuerSerial) is None:
        print "Error: failed to add %s node" % xmlsec.NodeX509IssuerSerial
        return cleanup(doc)
    ...
    #key = xmlsec.cryptoAppPkcs12Load(key_file, None, None, None)
