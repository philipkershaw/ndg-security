...
class attAuthority(ServiceSOAPBinding):
    soapAction = {
        'urn:attAuthority#getHostInfo': 'soap_getHostInfo',
    ...
    def soap_getHostInfo(self, ps):
        ...
        reqArgs = ps.Parse(hostInfoRequestWrapper)
        reqTxt = str(reqArgs._hostInfoReq)
        
        # assign return values to response object
        resp = hostInfoResponseWrapper()
        
        
        try:
            # Decrypt and parse input
            reqKeys = HostInfoReq(xmlTxt=reqTxt,
                                  encrPriKeyFilePath=self.__srv['keyFile'],
                                  encrPriKeyPwd=self.__srv['keyPwd'])
                     
            hostInfoResp = HostInfoResp(thisHost=self.__srv.hostInfo) 
                                         
        except Exception, e:
            hostInfoResp = HostInfoResp(errMsg=str(e))
        ...
        try:
            ...
            if 'encrCert' in reqKeys:
                ...
                hostInfoResp.encrypt(encrPubKeyFilePath=encrCertTmpFile.name)
                
        except Exception, e:
            hostInfoResp = HostInfoResp(statCode=hostInfoResp.accessError,
                                        errMsg=str(e))

        
        # Convert response into encrypted XML formatted string    
        resp._hostInfoResp = hostInfoResp()
        return resp
    ...
    def soap_getTrustedHostInfo(self, ps):
        ...
        try:
            ...
            trustedHosts = self.__srv.getTrustedHostInfo(role=role)
