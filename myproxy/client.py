...
class MyProxyServerSSLCertVerification(object):
    ...
    __slots__ = ('__hostname', '__cnPrefix', '__certDN')
    
    def __init__(self, 
                 certDN=None,
                 hostname=None,
                 cnPrefix=SERVER_CN_PREFIX):
    ...
    def _getCNPrefix(self):
        """References SSL Certificate verification object property!"""
        return self.__cnPrefix
    
    def _setCNPrefix(self, val):
        """Sets SSL Certificate verification object property!"""
        if not isinstance(val, basestring):
            raise TypeError("Expecting string type for hostname "
                                 "attribute")
        self.__cnPrefix = val
    
    cnPrefix = property(fget=_getCNPrefix,
                        fset=_setCNPrefix,
                        doc="Prefix for MyProxy Server Certificate "
                            "Distinguished Name CommonName field; usually set "
                            "to 'host/' for Globus host certificates")
...
TRUSTED_CERTS=1"""
    ...
    def _getServerCNPrefix(self):
        """References SSL Certificate verification object property!"""
        return self.__serverSSLCertVerify.cnPrefix
    
    def _setServerCNPrefix(self, val):
        """Sets SSL Certificate verification object property!"""
        self.__serverSSLCertVerify.cnPrefix = val
    
    serverCNPrefix = property(fget=_getServerCNPrefix,
                              fset=_setServerCNPrefix,
                              doc="Prefix for MyProxy Server Certificate "
                                  "Distinguished Name CoomonName field; "
                                  "usually set to host/ for Globus host "
                                  "certificates")
