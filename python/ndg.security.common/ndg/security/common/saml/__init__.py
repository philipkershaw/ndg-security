"""Implementation of SAML 1.1 Attribute Assertion for NDG Security
...
class AttributeAssertionError(Exception):  
    """Exception handling for NDG Attribute Assertion class."""

#_____________________________________________________________________________
class AttributeAssertionReadOnlyDict(dict):
    def __init__(self, inputDict):
        super(AttributeAssertionReadOnlyDict, self).__init__(inputDict)
        
    def __setitem__(self, key, item):
        raise KeyError("Items are read-only in this dictionary")
       
#_____________________________________________________________________________
class _MetaAttributeAssertion(type):
    """Enable AttributeAssertion to have read only class variables e.g.
    
    print AttributeAssertion.mappedProvenance is allowed but,
    
    AttributeAssertion.mappedProvenance = None
...
class AttributeAssertion(dict, XMLSecDoc):
    ...
    @cvar __validProvenanceSettings: string constants for allowable certificate
    provenance settings
    
    @type namespace: string
    @cvar namespace: namespace for Attribute Assertion"""
    
    __metaclass__ = _MetaAttributeAssertion
    ...
    def __init__(self, provenance='original', lifetime=28800, **xmlSecDocKw):
        """Initialisation - Attribute Assertion file path may be specified.
        ...
        self.__dat = {
            ...
            "version":            AttributeAssertion.version,
            "holder":             '',
            "issuer":             '',
            "issuerName":         '',
    ...
    def __delitem__(self, key):
        "Attribute Assertion keys cannot be removed"
        
        raise AttributeAssertionError('Keys cannot be deleted from ' + \
                                      AttributeAssertion.__name__)
    ...
    def __getitem__(self, key):
        ...
        dictionary of Attribute Assertion properties
        ...
        if key in self.__dat:
            ...
            raise KeyError('Key "%s" not recognised for %s' % \
                               (key, AttributeAssertion.__name__))

        if isinstance(item, dict):
            return AttributeAssertionReadOnlyDict(item)
        else:
            return item
    ...
    #_________________________________________________________________________    
    def __eq__(self, attributeAssertion):
        """Return true if all elements are the same"""        
        try:
            return min([self.__dat[key] == attributeAssertion[key] \
    ...
    def __nonzero__(self):
        """Ensure if <attributeAssertionInstance> test yields True"""
        return True
    
    
    #_________________________________________________________________________    
    def clear(self):
        raise AttributeAssertionError("Data cannot be cleared from " + \
                                      AttributeAssertion.__name__)
    ...
    holder = property(fget=__getHolder,
                      fset=__setHolder, 
                      doc="Attribute Assertion holder DN")
    ...
    holderDN = property(fget=__getHolderDN,
                        doc="Attribute Assertion holder DN as X500DN type")
    
    #_________________________________________________________________________    
    def __setIssuer(self, issuer):
        """Set issuer's Distinguished Name."""
        if not isinstance(issuer, basestring):
            raise AttributeError("issuer DN must be a string")
    ...
    issuerDN = property(fget=__getIssuerDN,
                        doc="Attribute Assertion issuer DN as X500DN type")
    ...
    roleSet = property(fget=__getRoleSet, doc="Role set dictionary")
    ...
    roles = property(fget=__getRoles, 
                     doc="List of roles in Attribute Assertion")
    ...
    def isValidProvenance(self, provenance=None):
        ...
        return provenance in AttributeAssertion.__validProvenanceSettings
    ...
    def isOriginal(self):
        ...
        return self.__dat['provenance'] == AttributeAssertion.origProvenance
    ...
    def isMapped(self):
        ...
        return self.__dat['provenance'] == AttributeAssertion.mappedProvenance
    ...
    def parse(self, xmlTxt, rtnRootElem=False):
        """Parse an Attribute Assertion content contained in string input

        @param xmlTxt:     Attribute Assertion XML content as string
    ...
    def __parse(self, rootElem):
        ...
        if not acInfoElem:
            raise AttributeAssertionError("<acInfo> tag not found in \"%s\"" % \
                               self.filePath)
        ...
        for elem in acInfoElem:
            if elem.tag not in self.__dat:
                raise AttributeAssertionError('%s: "<%s>" not recognised.' % \
        ...
        try:
            ...
            raise AttributeAssertionError("Issuer DN: %s" % x500dnErr)
        ...
        try:
            ...
            raise AttributeAssertionError("Holder DN: %s" % x500dnErr)
        ...
        if self.__dat['validity']['notBefore'] is None:
            raise AttributeAssertionError('<notBefore> tag not found in "%s"'%\
                                                               self.filePath)
        ...
        if self.__dat['validity']['notAfter'] is None:
            raise AttributeAssertionError('<notAfter> tag not found in "%s"' % \
                               self.filePath)
        elif self.__dat['validity']['notBefore'] == '':
        ...
        if roleElem is None:
            raise AttributeAssertionError("<role> tag not found in \"%s\"" % \
                               self.filePath)
        ...
        if not self.isValidVersion():           
            raise AttributeAssertionError('Attribute Assertion version is ' + \
                               self.__dat['version'] + ' but version ' + \
                               AttributeAssertion.version + ' expected')
    ...
    def _createXML(self):
        ...
        (inherited from XMLSecDoc) and pass the attributeAssertion object 
        reference into str()
    ...
    def setValidityTime(self,
        ...
        which the Attribute Assertion is valid.  These times are set as
        ...
        if dtNotAfter is not None:
            ...
            if dtDeltaLifeTime < timedelta(0):
                raise AttributeAssertionError("Input Not After time is invalid %s" % \
                                   str(dtNotAfter))
            ...
            try:
                ...
                raise AttributeAssertionError("Invalid Certificate lifetime set %.3f" %\
                                   self.__lifetime)
    ...
    def datetime2timeStr(self, dtVal):
        ...
        @rtype: string
        @return: datetime converted into correct string format for AttributeAssertion"""

        if not isinstance(dtVal, datetime):
            raise AttributeAssertionError("Invalid datetime object for "
    ...
    def timeStr2datetime(self, sTime):
        ...
        @param sTime: time in string format as used by AttributeAssertion
        @rtype: datetime
    ...
    def isValidTime(self, dtNow=None, raiseExcep=False):
        """Check Attribute Assertion for expiry.  Set raiseExcep to True
        ...
        if not isinstance(self.__dtNotBefore, datetime):
            raise AttributeAssertionError("Not Before datetime is not set")

        if not isinstance(self.__dtNotAfter, datetime):
            raise AttributeAssertionError("Not After datetime is not set")
        ...
        if raiseExcep:
            if dtNow < self.__dtNotBefore:
                raise AttributeAssertionError("Current time %s " % \
                           dtNow.strftime("%d/%m/%Y %H:%M:%S") + \
                           "is before Attribute Assertion's " + \
            ...
            if dtNow > self.__dtNotAfter:
                raise AttributeAssertionError("Current time %s " % \
                           dtNow.strftime("%d/%m/%Y %H:%M:%S") + \
                           "is after Attribute Assertion's " + \
                           "expiry time of %s" % \
                           self.__dtNotBefore.strftime("%d/%m/%Y %H:%M:%S"))               
    ...
    def isValidVersion(self):
        """Check Attribute Assertion XML file version
        
        @rtype: bool
        @return: boolean True if certificate version matches the expected one,
        False otherwise.
        """
        return self.__dat['version'] == AttributeAssertion.version
    ...
    def isValid(self,
                ...
                chkSig=True,
                **xmlSecDocKw):
        """Check Attribute Assertion is valid:
        ...
        @param chkVersion: set to True to Attribute Assertion file
        ...
        certificate used to sign the AttributeAssertion.  Alternatively, certFilePathList
        ...
        if chkProvenance and not self.isValidProvenance():
            if raiseExcep:
                raise AttributeAssertionError(\
                    "Attribute Assertion Provenance must be set to \"" + \
                    "\" or \"".join(AttributeAssertion.__validProvenanceSettings) + "\"")
            return False
        ...
        if chkSig:
            try:
                self.verifyEnvelopedSignature(**xmlSecDocKw)
                 ...
                 if raiseExcep:
                     raise AttributeAssertionError(e)
                 else:
                     return False
...
#_____________________________________________________________________________
# Alternative AttributeAssertion constructors
def AttributeAssertionRead(filePath):
    """Read an attribute assertion from a file"""
    
    attributeAssertion = AttributeAssertion(filePath=filePath)
    attributeAssertion.read()
    
    return attributeAssertion

#_________________________________________________________________________    
def AttributeAssertionParse(attributeAssertionTxt):
    """Parse an attribute assertion from a string"""
    
    attributeAssertion = AttributeAssertion()
    attributeAssertion.parse(attributeAssertionTxt)
...
def create():
    ...
    assertionElem = ElementTree.Element("{%s}Assertion" % ns)
    assertionElem.set('xmlns:%s' % nsPfx, ns)
    assertionElem.set('MajorVersion', '1')
    assertionElem.set('MinorVersion', '1')
    assertionElem.set('Issuer', issuer)

    conditionsElem = ElementTree.SubElement(assertionElem,
                                            '{%s}Conditions' % ns)    
    conditionsElem.set('NotBefore', 'x')
    conditionsElem.set('NotAfter', 'y')
    ...
    return tostring(assertionElem)
    
from StringIO import StringIO
def tostring(elem, **kw):
    f = StringIO()
    ElementC14N.write(ElementC14N.build_scoped_tree(elem), 
                      f, 
                      **kw)
    return f.getvalue()
