...
class ProftpPDP(PDPInterface):
    ...
    def __init__(self, cfg=None, **cfgKw):
        ...
        @param securityElement: directory containing a .ftpaccess file
        """
    ...
    def _readConfig(self, section='DEFAULT'):
        ...
        for paramName, paramVal in ProftpPDP.defParam.items():
            if isinstance(paramVal, list):
                paramListVal = expVars(cfg.get(section, paramName)).split()
                setattr(self, paramName, paramListVal)
            else:
                setattr(self, paramName, expVars(cfg.get(section, paramName)))            
    ...
    def _checkProFTPResrcConstraints(self, resrcHandle, attCert):
        ...
        for allowedGroup in allowedGroups:
             if allowedGroup in userRoles:
                 log.info(\
            'ProftpPDP: User role "%s" is in .ftpaccess allowed groups: %s' % \
                     (allowedGroup, ', '.join(allowedGroups)))
                 return
        ...
        for requiredGroupSet in requiredGroupSets:
            ...
            if userHasAllGroups:
                log.info(\
            'ProftpPDP: User has all the required .ftpaccess groups: %s' % \
                     ', '.join(requiredGroups))
                return
        ...
        if username in allowedUsers:
            log.info(\
                 'ProftpPDP: user ID "%s" is in list of allowed users: "%s"' %\
                 (username, '", "'.join(allowedUsers)))
            return
        
        
        # Catch all - default to deny access
        log.info(\
            'Access denied to resource %s for user "%s" with roles "%s"' % \
    ...
    def _pullUserSessionAttCert(self):
        ...
        try:
            ...
            log.info(\
            "ProftpPDP - request for attribute certificate denied: %s" % e)
    ...
    def _checkAttCert(self, attCert):
        ...
        if attCert.issuerDN != acIssuerDN:
            log.info('ProftpPDP -access denied: Attribute Certificate ' + \
