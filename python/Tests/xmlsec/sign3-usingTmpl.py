...
def sign_file(xml_file, key_file, cert_file):
    ...
    # Find start node
    signNode = xmlsec.findNode(doc.getRootElement(), xmlsec.NodeSignature,
                               xmlsec.DSigNs)
    if signNode is None:
...
##        return cleanup(doc)
##
##    # P J Kershaw 11/08/05
##    if keyInfoNode.addKeyName() is None:
##        print "Error: failed to add KeyName node"
##        return cleanup(doc)
##
####    if keyInfoNode.addKeyValue() is None:
####        print "Error: failed to add KeyValue node"
####        return cleanup(doc)
##    
