#!/usr/bin/env python
#
# $Id: verify3.py,v 1.1 2005/04/27 08:39:37 pjkersha Exp $
#
...
def verify_file(mngr, xml_file):
    ...
    node = xmlsec.findNode(rootElem, xmlsec.NodeSignature, xmlsec.DSigNs)
    if node is None:
    ...
    # P J Kershaw 08/11/05 - get X509Data tag info
    keyInfoNode = xmlsec.findNode(node, xmlsec.NodeKeyInfo, xmlsec.DSigNs)
    if keyInfoNode is None:
        print "Error: KeyInfo node not found in \"%s\"" % xml_file
        
    x509DataNode = xmlsec.findNode(keyInfoNode,
                                   xmlsec.NodeX509Data,
                                   xmlsec.DSigNs)
    if x509DataNode is None:
        print "Error: X509DataNode node not found in \"%s\"" % xml_file

    X509SubjectName = xmlsec.findNode(x509DataNode,
                                      xmlsec.NodeX509SubjectName,
                                      xmlsec.DSigNs)

    print "%s: %s" % (xmlsec.NodeX509SubjectName, X509SubjectName.content)
