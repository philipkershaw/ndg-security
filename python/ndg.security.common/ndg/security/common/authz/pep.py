...
class PEP(object):
    ...
    def initPDPInterface(self):
        ...
        if not issubclass(pdpClass, PDPInterface):
            raise PDPInitError, \
                "PDP interface class %s must be derived from PDPInterface" % \
                    self.pdpClassName
    ...
    def __call__(self, resrcHandle, userHandle, accessType, *arg, **kw):
        ...
        *arg and **kw are included to enable further customisation,
        ...
        if self._pdp is None:
            raise PDPInitError("PDP object is not set - " + \
                "ensure initPDPInterface has been called and the relevant " + \
