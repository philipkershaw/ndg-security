...
import unittest
from ndg.security.AttAuthorityIO import *
...
-----END CERTIFICATE-----
    ...
    def testSetHostInfoResponse(self):
        
        hi = {\
              'Gabriel': \
              {
              'wsdl': 'http://gabriel.bnsc.rl.ac.uk/attAuthority.wsdl', 
              'loginURI': 'https://gabriel.bnsc.rl.ac.uk/cgi-bin/security.py'
              }
          }
        ...
        pdb.set_trace()
        try:       
            self.hostInfoResp = HostInfoResp(thisHost=hi)
        except Exception, e:
            self.fail(str(e))
            
        print self.hostInfoResp()
        print self.hostInfoResp['thisHost']
...
<TrustedHostInfoResp>
    <trustedHosts>
        <trusted name="BADC">
            <wsdl>http://glue.badc.rl.ac.uk/attAuthority.wsdl</wsdl>
            <loginURI>https://glue.badc.rl.ac.uk/cgi-bin/security.py</loginURI>
            <roleSet>
                <role>school</role>
                <role>rapid</role>
                <role>government</role>
                <role>badcUser</role>
                <role>nercFunded</role>
                <role>bodcUser</role>
                <role>commercial</role>
                <role>metOfficeForm</role>
                <role>other</role>
                <role>acsoe</role>
