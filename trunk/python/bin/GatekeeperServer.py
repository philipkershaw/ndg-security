...
class GatekeeperSOAPRequestHandler(SOAPRequestHandler):
     ...
     def do_GET(self):
         ...
         wsdlfile = os.path.expandvars("$NDG_SECURITY_WSDL_DIR/gatekeeper.wsdl")
...
def runInForegnd():
    ...
    try:
        ...
        print >>sys.stderr, "Gatekeeper Server socket error: %s" % e[1]
        sys.exit(1)
        
    except Exception, e:
        print >>sys.stderr, "Gatekeeper Server: %s" % str(e)
        sys.exit(1)        
...
if __name__ == '__main__':
    ...
    gkPortNumEnvVarName = 'NDG_GK_PORT_NUM'
    defaultPort = 5200
    
    initPort = gkPortNumEnvVarName in os.environ and \
                        int(os.environ[gkPortNumEnvVarName]) or defaultPort
...
"set to stop in debugger on receipt of WS request.  %s flag must be set also"\
    ...
    try:
        ...
        print >>sys.stderr, "Initialising Gatekeeper: %s" % str(e)
        sys.exit(1)
    ...
    if options.foregndProc:
        ...
        if options.debug:
            print >>sys.stderr, "%s must be set for debug option" % \
                                                        '/'.join(foregndFlags)
            parser.print_help()
            sys.exit(1)
