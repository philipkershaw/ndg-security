...
#_____________________________________________________________________________
class HostInfoReqError(XMLMsgError):    
    """Exception handling for NDG AttAuthority WS GetHostInfo request 
    class."""
    pass


#_____________________________________________________________________________
class HostInfoReq(XMLMsg):
    """For client to Attribute Authority WS GetHostInfo(): formats 
...
#_____________________________________________________________________________
class HostInfoRespError(XMLMsgError):    
    """Exception handling for NDG AttAuthority WS GetHostInfo response 
    class."""
    pass


#_____________________________________________________________________________
class HostInfoResp(XMLMsg):                              
    ...
    def updateXML(self, **xmlTags):
        ...
        try:
            ...
            if "thisHost" in xmlTags:
                host, hostInfo = xmlTags['thisHost'].items()[0]
                xmlTxt += """
    <thisHost>
        <name>%s</name>
        <wsdl>%s</wsdl>
        <loginURI>%s</loginURI> 
    </thisHost>""" % (host, hostInfo['wsdl'], hostInfo['loginURI'])
    ...
    def parseXML(self):
        ...
        if 'thisHost' not in self:
            raise HostInfoRespError, \
                "\"thisHost\" tag is missing from the host info response"
        
        self['thisHost'] = {}
        
        hostElem = rootElem.find('thisHost')
        if not hostElem:
            raise HostInfoRespError, "No \"thisHost\" tag found in response"
         
        try:
            hostName = hostElem.find('name').text.strip()
            
            # Add key for trusted host name
            self['thisHost'][hostName] = {}
            
            # Add WSDL URI, loginURI and role set for that host
            self['thisHost'][hostName]['wsdl'] = \
                                        hostElem.find('wsdl').text.strip()
                                        
            self['thisHost'][hostName]['loginURI'] = \
                                        hostElem.find('loginURI').text.strip()              
        except Exception, e:
            raise HostInfoRespError, \
                        "Error parsing tag \"%s\" in host info response: %s" \
                        % (trusted.tag, str(e))
