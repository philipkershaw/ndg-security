    ...
    def createCookie(self, encrSessMgrWSDLuri, sessID=None, asString=True):
        ...
        try:
            ...
            for tagName in self.__cookieTagNames:
                ...
                # alone won't work
                sessCookie[tagName][self.__cookieDomainTagName] = '.ac.uk'#'glue.badc.rl.ac.uk'
...
class SessionMgr(object):
    ...
    def connect(self, reqXMLtxt=None, **connectReqKeys):        
        ...
        connect([webClnt=True/False][, reqXMLtxt]
                |[, userName=u, pPhrase=p]|[, proxyCert=px]|[, sessID=id])

        webClnt:                If True, allocate a user session with a 
        ...
        if 'sessID' in connectReqKeys:
            ...
            if 'webClnt' in connectReqKeys and connectReqKeys['webClnt']:
                
                # Return session cookies
                sessCookie = self.__createUserSession(proxyCert)
    ...
    def __createUserSession(self, proxyCert):
        ...
        and return cookie text for setting in browser client
...
#                    break
            ...
            # Return session cookies text
            return userSess.createCookie(self.encrSessMgrWSDLuri)
    ...
    def __connect2UserSession(self, **idKeys):
        ...
        if 'sessID' in idKeys:
            ...
            raise SessionMgrError(\
                "No user session found with ID: " + idKeys['sessID'])
            ...
            raise SessionMgrError(\
                    "No user session found with ID: " + idKeys['proxyCert'])
        else:
            raise SessionMgrError(\
    ...
    def reqAuthorisation(self, reqXMLtxt=None, **reqKeys):
        ...
        **reqKeys:              keywords used by 
        ...
        if userSessMgrWSDLuri != self.__prop['sessMgrWSDLuri']:
            ...
            try:
                traceFile = StringIO()
                smSrv = ServiceProxy(userSessMgrWSDLuri, 
                                     use_wsdl=True,
                                     tracefile=traceFile)
            except Exception, e:
                raise SessionMgrError(\
                    "Error initialising WS for \"%s\": %s\n\n%s" % \
                    (userSessMgrWSDLuri, e, traceFile.getvalue()))
        ...
        # User's session resides with THIS Session Manager ...
