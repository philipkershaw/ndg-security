...
class StoreCredException(Exception): pass
...
def myProxyStore(hostname, 
                 ...
                 port=7512):
    """
    Function to store credentials in a MyProxy server
    
    Exceptions:  GetException, StoreCredException
    """
...
#    context.use_privatekey_file(keyFile)   
    ...
    context.use_certificate_file('../hostcert.pem')
    context.use_privatekey_file('../hostkey.pem')   
    ...
    # send store command
    if debuglevel(1):  
        print "debug: send store command"
        
    cmd_store = CMD_STORE % (username, lifetime)
    conn.write(cmd_store)

    # process server response
    if debuglevel(1):   
        print "debug: get server response for store command request"
    ...
    certTxt = pat.findall(open(certFile).read())[0]
    ...
    # process server response
    if debuglevel(1):   
        print "debug: get server response for store command completed"
    resp = conn.recv(8192)
...
if __name__ == '__main__':
    ...
    parser.add_option("-c", "--store-cert", dest="certFile", default=None,
                       help="Certificate to be stored")
    
    parser.add_option("-y", "--store-key", dest="keyFile", default=None,
