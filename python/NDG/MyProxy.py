...
class MyProxy:
    ...
    def readProperties(self, propFilePath=None, propElem=None):
        ...
        if prop.has_key('simpleCACltProp'):
            ...
            prop['simpleCACltProp'] = dict(simpleCAClt)
            
        elif prop.has_key('simpleCASrvProp'):
            ...
            prop['simpleCASrvProp'] = dict(simpleCA)
...
""" % reqDN + cn + os.linesep
        ...
        if retDN:
            try:
                ...
                # the new user's DN
                user['dn'] = X509CertRead(certFile.name).getDN().serialise()
