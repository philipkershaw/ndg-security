...
def load_cert(file, **kw):
    ...
    return load_cert_bio(bio, **kw)


TYPE_PEM=0
TYPE_ASN1=1

def load_cert_bio(bio, type=TYPE_PEM):
    """Load certificate from PEM or ASN1 file format"""
    if type == TYPE_ASN1:
        return X509(m2.x509_read_asn1(bio._ptr()), 1)
    else:
        return X509(m2.x509_read_pem(bio._ptr()), 1)


def load_cert_string(string, **kw):
...
class Request:
    ...
    def as_asn1(self):
        buf=BIO.MemoryBuffer()
        m2.x509_req_write_asn1(buf.bio_ptr(), self.req)
        return buf.read_all()

    def save_asn1(self, filename):
        bio=BIO.openfile(filename, 'wb')
        return m2.x509_req_write_asn1(bio.bio_ptr(), self.req)
