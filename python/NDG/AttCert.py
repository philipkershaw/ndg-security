...
from NDG.xmlSigDoc import *

from NDG.X509 import X500DN
from NDG.X509 import X500DNError
...
class AttCert(UserDict, xmlSigDoc):
    ...
    def __init__(self, filePath=None, lifeTime=-1, **xmlSigDocArgs):
    ...
    def __delitem__(self, key):
        ...
        raise AttCertError('Keys cannot be deleted from ' + AttCert.__name__)


    def __getitem__(self, key):

        AttCert.__name__ + """ behaves as data dictionary of Attribute
    ...
    def clear(self):
        raise AttCertError("Data cannot be cleared from " + AttCert.__name__)
    ...
    def parse(self, xmlTxt):
        ...
        try:
            ...
            raise AttCertError(str(xmlSigDocErr))
    ...
    def __parse(self, rootElem):
        ...
        if not acInfoElem:
            raise AttCertError("<acInfo> tag not found in \""+filePath+"\"")
        ...
        for elem in acInfoElem:
            ...
            if not self.__dat.has_key(elem.tag):
                raise AttCertError(self.getFilePath() + "\": <" + \
                                   elem.tag + "> not recognised.")

            # Make sure not to copy validity and attributes tags - handle these
            # separately below
        ...
        if self.__dat['validity']['notAfter'] is None:
            raise AttCertError("<notAfter> tag not found in \"%s\""%filePath)
        ...
        if roleElem is None:
            raise AttCertError("<role> tag not found in \"" + filePath + \
                                "\"")
...
"""<attributeCertificate>
    ...
    def setValidityTime(self,dtNotBefore=None, dtNotAfter=None, lifeTime=-1):
        ...
        if dtNotAfter is not None and isinstance(dtNotAfter, datetime):
    ...
    def isValidTime(self):
        ...
        """Check Attribute Certificate for expiry"""
    ...
    def isValid(self,
        ...
        Also accepts keyword arguments corresponding to xmlSigDoc.isValidSig:
        ...
        if chkTime and not self.isValidTime():
            if raiseExcep:
                raise AttCertError("Attribute Certificate time is invalid")
        ...
        # Handle exception from xmlSigDoc.isValidSig() regardless of
        # raiseExcep flag setting
        try:            
            if chkSig and not self.isValidSig(**xmlSigDocKeys):
                if raiseExcep:
                    raise AttCertError(\
