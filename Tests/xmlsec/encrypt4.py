#!/usr/bin/env python
#
# $Id: encrypt3.py,v 1.3 2004/01/25 00:31:12 valos Exp $
#
...
def main():
    ...
    if len(sys.argv) != 4:
        ...
        print "Usage: %s <xml-file> <PEM Cert file> <PEM Private Key file>" % sys.argv[0]
        return sys.exit(1)
    ...
    mngr = load_rsa_keys(sys.argv[2], sys.argv[3])
...
def load_rsa_keys(pemCertFilePath, pemPriKeyFilePath):
    assert(pemCertFilePath)
    ...
    if not check_filename(pemCertFilePath):
        mngr.destroy()
        return None
#    key = xmlsec.cryptoAppKeyLoad(pemCertFilePath, xmlsec.KeyDataFormatPem,
#                                  None, None, None);
    key = xmlsec.cryptoAppKeyLoad(pemCertFilePath, 7,
                                  None, None, None);
    if key is None:
        print "Error: failed to load rsa key from file \"%s\"" % pemCertFilePath
        mngr.destroy()
        return None
    # Set key name to the file name, this is just an example!
    if key.setName(pemPriKeyFilePath) < 0:
        print "Error: failed to set key name for key from \"%s\"" % pemPriKeyFilePath
        key.destroy()
        mngr.destroy()
        return None
...
def encrypt_file(mngr, xml_file, key_name):
    ...
    # P J Kershaw 06/09/05
    x509DataNode = key_info_node2.addX509Data()
    if x509DataNode is None:
        print "Error adding X509Data node"
        cleanup(doc, enc_data_node)

    if xmlsec.addChild(x509DataNode,
                       xmlsec.NodeX509Certificate) is None:
        print "Error adding %s node" % xmlsec.NodeX509Certificate
        cleanup(doc, enc_data_node)
        
    if xmlsec.addChild(x509DataNode,
                       xmlsec.NodeX509SubjectName) is None:
        print "Error adding %s node" % xmlsec.NodeX509SubjectName
        cleanup(doc, enc_data_node)
        
    if xmlsec.addChild(x509DataNode,
                       xmlsec.NodeX509IssuerSerial) is None:
        print "Error adding %s node" % xmlsec.NodeX509IssuerSerial
        cleanup(doc, enc_data_node)
