...
from ndg.security.server.zsi.twisted.sessionmanager.SessionManager_services_server\
...
from ndg.security.server.zsi.twisted import WSSecurityHandler, \
...
class SessionManagerService(_SessionManagerService, WSResource):
    ...
    def soap_connect(self, ps, **kw):
        ...
        request, response = _SessionManagerService.soap_connect(self, ps)
    ...
    def soap_disconnect(self, ps, **kw):
        ...
        request, response = _SessionManagerService.soap_disconnect(self, ps)
        ...
        if srv.sm.has_key('WS-Security'):
            # Get certificate corresponding to private key that signed the
            # message - i.e. the user's proxy
            userX509Cert = WSSecurityHandler.signatureHandler.verifyingCert
        else:
    ...
    def soap_getSessionStatus(self, ps, **kw):
        ...
        request, response = _SessionManagerService.soap_getSessionStatus(self, ps)
    ...
    def soap_getAttCert(self, ps, **kw):
        ...
        request, response = _SessionManagerService.soap_getAttCert(self, ps)
    ...
    def soap_getX509Cert(self, ps, **kw):
        ...
        request, response = _SessionManagerService.soap_getX509Cert(self, ps)

        x509Cert = X509CertRead(srv.sm['signingCertFilePath'])
        response.X509Cert = base64.encodestring(x509Cert.asDER())
        return request, response


# Create Service
srv = SessionManagerService()
...
if srv.sm.has_key('WS-Security'):
	...
	refC14nInclNS = srv.sm['refC14nInclNS']
	signedInfoC14nInclNS = srv.sm['signedInfoC14nInclNS']

	WSSecurityHandler.signatureHandler = SignatureHandler(\
							verifyingCertFilePath=srv.sm['verifyingCertPath'],
                            signingCertFilePath=srv.sm['signingCertFilePath'],
                            signingPriKeyFilePath=srv.sm['signingPriKeyFilePath'],
                            signingPriKeyPwd=srv.sm['signingPriKeyPwd'],
                            caCertFilePathList=srv.sm.get('caCertFilePathList'),
