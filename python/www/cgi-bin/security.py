...
#             extTrustedHost = ''
	...
	elif form.has_key("attCert"):
	    self.showAttCert(attCert=form['attCert'].value)
...
"""<script>alert("Login error%s: %s")</script> 
    ...
    #_________________________________________________________________________
    def authorise(self, cookie=None, reqRole=None):
	    ...
	    elif self.__authorisationMethod == 'allowMappingWithPrompt':
		...
		bRtnExtAttCertList = False
            ...
            try:
                ...
                resp = smClient.reqAuthorisation(sessCookie=cookieObj,
                                    aaWSDL=self.__aaWSDL,
				                    aaPubKey=self.__aaPubKey,
                                    reqRole=reqRole,
            ...
            if resp['statCode'] == 'AccessGranted':
                ...
                if 'extAttCertList' not in resp or not resp['extAttCertList']:
...
<head>
    ...
    <script language="JavaScript">
	<!--
	function ndgDataOpenWindow(theURL, winName, features)
	{ //v2.0
...
if __name__ == "__main__":
    ...
    try:
    	aaPubKey=open("...cert.pem").read()

    except IOError, (errNo, errMsg):
        raise Exception("Making temporary file for Attribute " + \
    ...
    smPubKeyFilePath = "...cert.pem"
    clntPubKeyFilePath = "...-cert.pem"
    clntPriKeyFilePath = "...-key.pem"
